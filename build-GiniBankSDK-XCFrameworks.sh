...
# Creates a device-only XCFramework (arm64, no simulator slice).
# Reuses the iphoneos archive — no extra build step needed.
...
make-slim-xcframework() {
    ...
    echo "Creating slim XCFramework for $fr_name (device arm64 only, no simulator slice)"
    xcodebuild -create-xcframework \
        -framework "$framework_path" \
...
for framework in "${frameworks[@]}"; do
    make-slim-xcframework "$framework" "$iphoneos_archive_path"
done
