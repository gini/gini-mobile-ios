...
final class ComponentAPICoordinator: NSObject, Coordinator {
    ...
    fileprivate(set) lazy var multipageReviewScreen: ReviewViewController = {
    ...
    init(pages: [GiniCapturePage],
         ...
         documentService: ComponentAPIDocumentServiceProtocol, giniHealth: GiniHealth) {
        self.pages = pages
        ...
        documentService?.startAnalysis(completion: { doc, result   in
            DispatchQueue.main.async {
                ...
                case .success(let extractions):
                    self.handleAnalysis(document: doc!, giniHealth: self.giniHealth, extractions: extractions)
                case .failure( _):
...
// MARK: GiniHealth SDK Handle analysis results open PaymentReviewView screen
...
extension ComponentAPICoordinator: GiniHealthDelegate {
