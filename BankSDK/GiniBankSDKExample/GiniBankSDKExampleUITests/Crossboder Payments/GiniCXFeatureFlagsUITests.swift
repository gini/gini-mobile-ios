...
// 2. Place skonto_past.pdf and return_asistant.pdf in the device Files app
//    (BrowserStack: upload both via Custom_Files before running).
...
class GiniCXFeatureFlagsUITests: GiniBankSDKExampleUITests {
    ...
    /*
     To launch these tests and closely mimic real user behaviour
     Please place in the device Files app:
         "skonto_past"      — a skonto invoice (C1: verifies Skonto is suppressed in CX mode)
         "return_asistant"  — a return assistant invoice (C2: verifies RA is suppressed in CX mode)
     */
    ...
    func testCXSkontoScreenIsNotShown() {
        ...
        //Tap Upload files button and pick the skonto invoice PDF
    ...
    func testCXReturnAssistantScreenIsNotShown() {
        ...
        //Tap Files button
        captureScreen.filesButton.tap()
        //Tap Upload files button and pick the Return Assistant invoice PDF
        captureScreen.uploadFilesButton.tap()
        mainScreen.tapFileFromBestAvailableSource(fileName: TestFixtures.Files.skontoPast)
        //Open button appears on some iOS versions/flows; safe to skip if absent.
        if captureScreen.openGalleryButton.waitForExistence(timeout: 3) {
