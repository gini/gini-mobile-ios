...
The Gini Merchant SDK for iOS provides all the UI and functionality needed to use the Gini Health API in your app to extract payment and merchant information from invoices. The payment information can be reviewed and then the invoice can be paid using any available payment provider app (e.g., banking app).

The Gini Health API provides an information extraction service for analyzing merchant invoices. Specifically, it extracts information such as the document sender or the payment relevant information (amount to pay, IBAN, etc.). In addition it also provides a secure channel for sharing payment related information between clients. 
...
Now that the `GiniHealthAPI` has been initialized, you can initialize `GiniMerchant`:
...
For the document upload if you plan to do it with `GiniMerchant`. First you need get document service and create partial document.

```swift
let documentService: DefaultDocumentService = merchantSDK.documentService()
...
self.merchantSDK.documentService
...
GiniMerchant provides a method for checking if the document is payable or not.

```swift
merchantSDK.checkIfDocumentIsPayable(docId: String,
                                   completion: @escaping (Result<Bool, GiniMerchantError>) -> Void)
```
