...
**R8 (MUST, happy):** Given the Analysis screen displays an image document and the `CaptureSuggestionsView` banner is presented (pinned to bottom safe area) AND the badge is visible, when the banner appears, then the banner sits above the badge (banner's bottom anchor pins to the badge's top with `Constants.badgeSuggestionsGap`), the badge never occludes the banner, and `removeCaptureSuggestions()` on `viewWillDisappear` leaves the badge in place.
