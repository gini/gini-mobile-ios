...
R4 (MUST, happy): Given the Gini loading indicator view is instantiated, when it lays out and renders, then it plays the HEIC (`HEICS`) frames from `GiniBrand.xcassets` in a smooth continuous loop (no visible pause at loop boundary, no flicker on start or on stop). Frames are decoded via `CGImageSourceCreateThumbnailAtIndex` (capped at `Constants.thumbnailMaxPixelSize` to keep the retained backing store well below the source resolution) and installed on `UIImageView.animationImages` alongside a total loop duration summed from per-frame `kCGImagePropertyHEICSUnclampedDelayTime` / `kCGImagePropertyHEICSDelayTime` (falling back to `kCGImagePropertyGIF*DelayTime` and finally `Constants.fallbackFrameDelay`).

R5 (MUST, entry): Given the app is in Dark Mode, when the Gini loading indicator view renders, then it displays the dark-appearance variant of the HEIC asset (`gini_loading_indicator_dark.dataset`); given Light Mode, it displays the light variant (`gini_loading_indicator_light.dataset`). The variant switch happens on `traitCollectionDidChange` when appearance flips at runtime, without a full view recreation, using the shared process-wide decoded-frame cache in `PoweredByGiniLoadingIndicatorView`.

R6 (MUST, happy): Given `ingredientBrandScreens` contains `"Analysis"` and `AnalysisViewController` enters the education-loading path (`showEducationLoadingMessage` → `QRCodeEducationLoadingView`), when each `QRCodeEducationLoadingItem` in the carousel is shown, then the item's `.image` (today `qrCodeEducationIntroIcon` / `qrCodeEducationUploadIcon`) is replaced by the animated Gini HEIC returned from `PoweredByGiniLoadingIndicatorView.animatedImage(for:)`, while the item's `.text` and the carousel's per-item `.duration` are preserved unchanged. The Figma reference shows the animated Gini mark in place of both education icons.
...
Public UIView, backed by an `UIImageView` playing the animated HEIC loaded from the shared brand asset catalog. Structure and file placement mirror the existing sibling `GiniComponents/Utilities/GiniUtilites/Sources/GiniUtilites/Brand/PoweredByGiniBadgeView.swift` line-for-line (imports, `public final class`, `imageView` child, autolayout in `init()`, `translatesAutoresizingMaskIntoConstraints = false`, `isAccessibilityElement`, `accessibilityLabel`, `accessibilityTraits`).

**Layout (per Figma sizing):** the child `UIImageView` is pinned to the view's edges via `giniMakeConstraints`; sizing is driven by each decoded frame's intrinsic content size (both placements render at the HEIC's exported pt size, no explicit `Constants.indicatorSize`):
...
**Dark mode (R5):** Two dedicated universal datasets ship in `GiniBrand.xcassets` — `gini_loading_indicator_light.dataset/giniLoadingIndicator-light.heic` and `gini_loading_indicator_dark.dataset/giniLoadingIndicator-dark.heic`. `decodeFrames(for style: UIUserInterfaceStyle)` picks the dataset by name from the current `userInterfaceStyle` and looks it up via `NSDataAsset(name:bundle:)`. On appearance flip at runtime, `traitCollectionDidChange(_ previous:)` re-invokes the decoder and swaps the animated image — no view recreation needed. Decoded frame sets are cached process-wide by style so repeat callers (education carousel + standalone view) share one decode; see Q11 for why the split is two datasets rather than one dataset with `luminosity` appearance variants.

**A11y (R8):** `isAccessibilityElement = true`, `accessibilityTraits = [.updatesFrequently, .image]`, and the `accessibilityLabel` inherited from `UIView` is set by both `AnalysisViewController` and `QRCodeOverlay` from `loadingIndicatorText.text` at install time so VoiceOver announces the localized loading message in every locale (replacing the hardcoded English `"Loading"` fallback). No new public accessibility property on the view.
...
**QRCodeOverlay's education path stays on `useIngredientBrandIndicator: false`.** The overlay's own `QRCodeEducationLoadingView` construction uses the default; the animated Gini mark appears on the overlay's standard (non-education) loading path only, via `installBrandedLoadingIndicatorIfNeeded()` — see Q9 for the QR overlay expansion and Q10 for the show-time deferral.
